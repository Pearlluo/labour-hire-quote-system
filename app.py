    ...
    for f in items:
        ...
        for _u in [url_job, url_commercial]:
            if _u:
                ...
                _m = _re.search(r'01[_ ]Marlu[_ ]Contracts[/\\%]([^/%?]+)', _unquote(str(_u)))
                if _m:
...
def transform_rates(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for f in items:
        ...
        if not project and not position and not title:
            continue
...
def transform_projects(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for f in items:
        ...
        lead = lookup_value(get_first(f, ["MarluProjectLead", "MarluProjectLeadId"]))
...
def main():
    parser = argparse.ArgumentParser(
        description="Marlu Online Quote Tool SharePoint Data Extractor"
    )
