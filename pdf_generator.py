...
from excel_generator import _calc_role_cost, _build_shift_map, _num, _day_type
...
def _rates_table(all_roles, width):
    """Full rate card: Weekday | Weekend | Public Holiday $/hr per role."""
    r0 = ["Role", "Qty", "ORD\nAfter", "Weekday", "", "", "", "Weekend", "", "", "", "Public\nHol."]
    r1 = ["", "", "", "DS Std", "DS ORD", "NS Std", "NS ORD",
          "DS Std", "DS ORD", "NS Std", "NS ORD", "$/hr"]
    data = [r0, r1]
    for r in all_roles:
        data.append([
            _s(r.get("role", "")), str(r.get("qty", 1)), f"{_num(r,'ord','ot') or 8:g}",
            _money(_num(r, "dsStd", "dsRate")), _money(_num(r, "dsOrd", "dsOT")),
            _money(_num(r, "nsStd", "nsRate")), _money(_num(r, "nsOrd", "nsOT")),
            _money(_num(r, "dsStdWk")), _money(_num(r, "dsOrdWk")),
            _money(_num(r, "nsStdWk")), _money(_num(r, "nsOrdWk")),
            _money(_num(r, "ph")),
        ])
    w = width
    rate = (w - w*0.16 - w*0.05 - w*0.06) / 9
    col = [w*0.16, w*0.05, w*0.06] + [rate]*9
    t = Table(data, colWidths=col, repeatRows=2)
    base = [
        ...
        ("SPAN", (11, 0), (11, 1)),
        ("BACKGROUND", (7, 2), (10, -1), WK_TINT),
        ("BACKGROUND", (11, 2), (11, -1), PH_TINT),
    ]
    style = _grouped_header(r0, r1, base, [(3, 6, NAVY), (7, 10, ORANGE), (11, 11, PURPLE)])
    t.setStyle(TableStyle(style))
    return t
...
def _manning_grid(all_roles, manning, shift_map, dates, multiplier, width):
    ...
    role_w, client_w = width * 0.16, width * 0.085
    ...
    for ri, r in enumerate(all_roles, start=1):
        ...
        for ci, d in enumerate(dates, start=1):
            ...
            if v not in ("OFF", "—", ""):
                if _day_type(d) == "ph":
                    cell_styles.append((ci, ri, PH_TINT))
                elif str(v).startswith("NS"):
    ...
    col = [role_w] + [day_w]*len(dates) + [client_w]
    ...
    # date header colours by day type
    for ci, dt in enumerate(day_types, start=1):
        bg = PURPLE if dt == "ph" else (WK_HDR if dt == "wk" else TEAL_MID)
        style.append(("BACKGROUND", (ci, 0), (ci, 0), bg))
...
def _alloc_table(all_roles, manning, shift_map, dates, multiplier, st, width):
    """Grouped per-role breakdown: Weekday | Weekend | Public Holiday (hrs + cost)."""
    r0 = ["Role", "Qty", "ORD", "Weekday", "", "", "", "", "Weekend", "", "", "", "",
          "Public Holiday", "", "", "Total $", "Client $"]
    r1 = ["", "", "", "DS Std", "DS ORD", "NS Std", "NS ORD", "Cost",
    ...
    for r in all_roles:
        ...
        data.append([
            ...
            f"{_num(r,'ord','ot') or 8:g}",
            f(c["wd"]["ds_std"]), f(c["wd"]["ds_ord"]), f(c["wd"]["ns_std"]), f(c["wd"]["ns_ord"]), _money(wd_cost),
            f(c["wk"]["ds_std"]), f(c["wk"]["ds_ord"]), f(c["wk"]["ns_std"]), f(c["wk"]["ns_ord"]), _money(wk_cost),
            str(c["ph_shifts"]), f(c["ph_hrs"]), _money(c["ph_cost"]),
            _money(c["total"]), _money(c["client_charge"]),
        ])
    data.append(["TOTALS"] + [""]*15 + [_money(tot_base), _money(tot_client)])

    w = width
    fixed = w*0.15 + w*0.04 + w*0.04
    small = (w - fixed) / 15
    col = [w*0.15, w*0.04, w*0.04] + [small]*15
    t = Table(data, colWidths=col, repeatRows=2)
    base = [
        ("FONTNAME", (0, 2), (-1, -1), "Helvetica"),
        ("FONTSIZE", (0, 2), (-1, -1), 6.3),
        ("ALIGN", (1, 2), (-1, -1), "CENTER"),
        ("ALIGN", (0, 2), (0, -1), "LEFT"),
        ("ALIGN", (7, 2), (7, -1), "RIGHT"), ("ALIGN", (12, 2), (12, -1), "RIGHT"),
        ("ALIGN", (15, 2), (-1, -1), "RIGHT"),
        ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
        ("GRID", (0, 0), (-1, -1), 0.3, GREY_LN),
        ...
        ("BACKGROUND", (8, 2), (12, -2), WK_TINT),
        ("BACKGROUND", (13, 2), (15, -2), PH_TINT),
        ("BACKGROUND", (0, -1), (-1, -1), TEAL),
        ("TEXTCOLOR", (0, -1), (-1, -1), colors.white),
        ("FONTNAME", (0, -1), (-1, -1), "Helvetica-Bold"),
        ("SPAN", (0, -1), (15, -1)),
    ]
    style = _grouped_header(r0, r1, base, [(3, 7, NAVY), (8, 12, ORANGE), (13, 15, PURPLE)])
    t.setStyle(TableStyle(style))
    return t
...
def _pricing_summary(payload, all_roles, manning, shift_map, dates, multiplier, st, width):
    ...
    for r in all_roles:
        c = _calc_role_cost(r, manning, shift_map, dates, multiplier)
        agg["wd_ds"] += c["wd_ds_cost"]; agg["wd_ns"] += c["wd_ns_cost"]
        agg["wk_ds"] += c["wk_ds_cost"]; agg["wk_ns"] += c["wk_ns_cost"]
        agg["ph"]   += c["ph_cost"]
    ...
    rows = [
        ("Weekday DS Cost", agg["wd_ds"], 0),
        ("Weekday NS Cost", agg["wd_ns"], 0),
        ("Weekend DS Cost", agg["wk_ds"], 0),
        ("Weekend NS Cost", agg["wk_ns"], 0),
        ("Public Holiday Cost", agg["ph"], 0),
...
def generate_quote_pdf(payload: Dict[str, Any]) -> bytes:
    st = _styles()
    ...
    inner = Table([
        ...
        [Paragraph(f"Job: {di.get('job_id','')}", st["sub"]),
         Paragraph(f"Generated: {generated}&nbsp;&nbsp;|&nbsp;&nbsp;Multiplier: ×{multiplier:.2f}", st["sub"])],
